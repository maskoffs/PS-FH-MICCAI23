...
from train.segment_anything.modeling.mask_decoder import MLP, MaskDecoder
import numpy as np
import sys
...
from train.segment_anything import sam_model_registry
import torch
import logging
import os
...
if __name__ == "__main__":
    ...
    with open("./train/train_fold_4.txt", "r") as file:
        lines = file.readlines()
    train_index = [int(line.strip().split("/")[-1]) - 1 for line in lines]
    with open("./train/val_fold_4.txt", "r") as file:
        lines = file.readlines()
    test_index = [int(line.strip().split("/")[-1]) - 1 for line in lines]
    print(len(train_index), len(test_index))

    snapshot_path = f'./train/train_ckpts/b{batch_size}_wd{weight_decay}_results/{fold_n}'
    ...
    sam, img_embedding_size = sam_model_registry['vit_h'](image_size=512,
                                                          num_classes=8,
                                                          checkpoint='./train/checkpoints/sam_vit_h_4b8939.pth',
                                                          pixel_mean=[0, 0, 0],
                                                          pixel_std=[1, 1, 1])
    ...
    """
    net.load_lora_parameters(f'./train/ckpts/{ckpts[-1]}')
    ...
    optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=b_lr, betas=(0.9, 0.999),
                            weight_decay=0.1)
    iter_num = 0
    writer = SummaryWriter(f'train/results/{fold_n}/logs')
