...
model = dict(
    ...
    decode_head=dict(
        num_classes=3,
        loss_decode=dict(
            type='AutoWeightedCELoss', use_sigmoid=False,
            class_weight=[1, 0.98425 / 0.53951, 0.88461 / 0.53951], loss_weight=1.0)
        ),
    auxiliary_head=dict(
        num_classes=3,
        loss_decode=dict(
            type='AutoWeightedCELoss', use_sigmoid=False,
            class_weight=[1, 0.98425 / 0.53951, 0.88461 / 0.53951], loss_weight=0.4)
    ),
)
